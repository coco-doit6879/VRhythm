...
export function Header({ active, loggedIn, theme, onToggleTheme, onNavigate, onSignOut }: Props) {
  ...
  return <header className="topbar">
    ...
    <nav className={open ? 'nav open' : 'nav'}>
      ...
      {loggedIn ? <button className="nav-account" onClick={onSignOut}><LogOut size={15} /> Thoát</button> : <button className={active === 'auth' ? 'active' : ''} onClick={() => go('auth')}><LogIn size={15} /> Đăng nhập</button>}
    </nav>
  </header>;
}
