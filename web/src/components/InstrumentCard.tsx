...
export function InstrumentCard({ instrument, onSelect }: { instrument: Instrument; onSelect: (instrument: Instrument) => void }) {
  ...
  return <button className="instrument-card" style={{ '--instrument-accent': instrument.accent } as React.CSSProperties} onClick={() => onSelect(instrument)} aria-label={`Mở hồ sơ văn hóa ${instrument.name}`}>
    <div className={`instrument-art${generated ? ' editorial-art' : ''}`}><img className={!generated && instrument.transparentImage ? 'transparent-instrument' : ''} src={generated ? `/images/generated/${generated}-editorial.webp` : instrument.transparentImage ?? instrument.image} alt="" loading="lazy" />{generated ? <small className="editorial-credit">Ảnh minh họa</small> : <span>{instrument.symbol}</span>}</div>
