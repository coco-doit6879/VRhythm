...
export default function App() {
  ...
  return <div className={`app-shell theme-${theme}`}><Header active={view} loggedIn={Boolean(authUser)} theme={theme} onToggleTheme={toggleTheme} onNavigate={value => navigate(value as View)} onSignOut={signOut} />
    ...
    {view === 'explore' && <Explore onInstrument={setSelectedInstrument} onLearn={() => navigate('learn')} />}
    {view === 'learn' && <Learn user={authUser} courses={courses.length ? courses : mockCourses} onAuth={() => openAuth('login')} onOpenLesson={openLesson} />}
    ...
    {selectedInstrument && <InstrumentModal instrument={selectedInstrument} onClose={() => setSelectedInstrument(null)} onLearn={() => { setSelectedInstrument(null); navigate('learn'); }} />}
  </div>;
}
...
function Learn({ user, courses, onAuth, onOpenLesson }: { user: AuthResponse | null; courses: Array<CourseSummary | typeof mockCourses[number]>; onAuth: () => void; onOpenLesson: (route?: LessonRoute) => void }) {
  const [instrument, setInstrument] = useState('Tất cả nhạc cụ');
  const setView = (_view: string) => onOpenLesson();
  const visibleCourses = instrument === 'Tất cả nhạc cụ' ? courses : courses.filter(course => course.instrument.toLocaleLowerCase() === instrument.toLocaleLowerCase());
  const instrumentOptions = ['Tất cả nhạc cụ', ...Array.from(new Set(courses.map(course => course.instrument)))];
  return <main className="page learn-page"><section className="dashboard-top"><div><div className="section-kicker">Bảng điều khiển học tập</div><h1>{user ? `Chào ${user.profile.fullName.split(' ')[0]}.` : 'Bắt đầu hành trình của bạn.'}</h1><p>{user ? 'Bạn đang tiến bộ rất đẹp. Cùng tiếp tục bài học hôm nay nhé.' : 'Đăng nhập để lưu lại tiến độ, XP, streak và thành tích của bạn.'}</p>{!user && <button className="primary" onClick={onAuth}>Đăng nhập để đồng bộ <UserRound size={16} /></button>}</div><div className="streak-card"><Flame size={22} /><strong>12</strong><span>ngày liên tục</span><small>Giữ nhịp thật đều nhé</small></div></section><section className="stat-row"><div><Trophy /><span>XP hiện tại</span><strong>1.240</strong></div><div><Target /><span>Tiến độ tổng</span><strong>28%</strong></div><div><BookOpen /><span>Bài đã học</span><strong>08 / 24</strong></div></section><section className="learning-layout"><div className="learning-main"><div className="section-heading inline"><div><div className="section-kicker">03 · Chọn nhạc cụ</div><h2>Những khóa học của bạn</h2></div><select value={instrument} onChange={event => setInstrument(event.target.value)}>{instrumentOptions.map(option => <option key={option}>{option}</option>)}</select></div><div className="course-list">{visibleCourses.map(course => <article className="course-card" key={course.id}><div className="course-card-top" style={{ background: 'color' in course ? course.color : '#4f8f83' }}><span>{'level' in course ? course.level : course.accessType}</span><b>{course.instrument}</b></div><div className="course-card-body"><h3>{course.title}</h3><p>{'duration' in course ? `${course.lessons} bài · ${course.duration}` : course.description}</p><div className="progress-label"><span>Tiến độ</span><strong>{'progress' in course ? course.progress : 0}%</strong></div><div className="progress-bar"><i style={{ width: `${'progress' in course ? course.progress : 0}%` }} /></div><button className="text-button" onClick={() => setView('lesson')}>Tiếp tục học <ArrowRight size={15} /></button></div></article>)}</div>{visibleCourses.length === 0 && <p className="muted">Chưa có khóa học cho nhạc cụ này.</p>}</div><aside className="path-card"><div className="section-kicker">Lộ trình học</div><h2>Khóa nhập môn<br /><em>sáo trúc</em></h2><div className="path-list">{learningPath.map(item => <div className={`path-item ${item.status}`} key={item.number}><span className="path-number">{item.status === 'done' ? <Check size={14} /> : item.status === 'locked' ? <Lock size={13} /> : item.number}</span><div><strong>{item.title}</strong><small>{item.type}</small></div>{item.status === 'current' && <span className="current-dot" />}</div>)}</div></aside></section></main>;
}
